...
class _WatchlistScreenState extends State<WatchlistScreen> {
  ...
  Future<void> _loadWatchlist() async {
    ...
    final ids = list.map((e) => int.tryParse(e) ?? 0).toSet();

    final allMovies = [
      ...TMDBService.fallbackMovies,
      ...await TMDBService.fetchPopular(),
    ];

    final matched = allMovies.where((m) => ids.contains(m.id)).toSet().toList();

    if (mounted) {
      setState(() {
        _savedMovies = matched;
        _isLoading = false;
      });
    }
  }
